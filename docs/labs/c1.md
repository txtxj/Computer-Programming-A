...
- `data.txt` 共 100 行，每行包含三个以 `0x` 开头的十六进制整数和一个小于 `10` 的十进制自然数 ，分别为 `p` `q` `blum` `pass MR-test time`，格式如下：
...
- 第一行数据表示：两素数分别为 `0x3` 和 `0x5` ，乘积（blum 数）为 `0xf` 。 blum 数 `0xf` 共通过了 `1` 次素性测试。